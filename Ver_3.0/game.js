...
window.onload = function() {
	...
	var Target = Class.create(Sprite, {
		initialize: function(from, scene) {
			...
			this.onenterframe = () => {
				if (!WorldFlg || deadFlgs[this.num]) return;
	...
	var TankBase = Class.create(Sprite, {
		...
		_Move: function(rot) {
			if (this._Rotation(rot)) {
				...
				this.moveBy(dx, dy);
				/*if(Aim.collection.length > 0){
					for (var i = 0, l = Aim.collection.length; i < l; i++) {
						let c = Aim.collection[i];
						if(c.num == this.num){
							c.moveBy(dx, dy);
						}
					}
		...
		_Damage: function(){
			...
			if (!this.damFlg) {
				...
				for (const elem of hits) {
					...
					this.life--;
					if (this.num == 0 && gameMode == 2) zanki--;
					from._Destroy();

...
var playerType = 0;
var playerLife = 0;
var useLifeSystem = false; // true: 体力制モード / false: 撃破数制モード
...
const Categorys = {
	...
	// 体力制モード用のHP値（useLifeSystem == true のとき使用）
	LifeHP: [
		50, //Player
		10, //brown
		10, //gray
	...
	// 撃破制モード用の耐久値（useLifeSystem == false のとき使用）
	LifeCount: [
		1, //Player
		1, //brown
		1, //gray
	...
	// 実行時参照用（useLifeSystemで切り替え）
	get Life() {
		return useLifeSystem ? this.LifeHP : this.LifeCount;
	},
	MaxBullet: [
		5, //Player
		1, //brown
...
window.onload = function() {
	...
	class AimLite {
		static list = [];
		constructor(from, to, category, num, ref, speed) {
			this.from = from;
			this.to = to;
		...
		update(from, to) {
			...
			if (Math.abs(diff) >= this.speed) {
				from.rotation = normalizeRotation(from.rotation + (diff > 0 ? -this.speed : this.speed));
			} else {
				from.rotation = rot;
			}
		...
		intersectsAABB(rect) {
			...
			// AABB の外側に完全にある場合は即 false
			if (s.x1 < rect.x && s.x2 < rect.x) return false;
			if (s.x1 > rect.x + rect.width && s.x2 > rect.x + rect.width) return false;
			if (s.y1 < rect.y && s.y2 < rect.y) return false;
			if (s.y1 > rect.y + rect.height && s.y2 > rect.y + rect.height) return false;

			return lineIntersectsRect(s.x1, s.y1, s.x2, s.y2, rect);
		}
	...
	class RefAimLite {
		...
		constructor(ref, from, category, num) {
			...
			//this.debugColor = "rgba(255, 0, 0, 0.4)";
	...
	function findBestHitAngle(ref, from, category, num, target) {
		...
		for (let deg = 0; deg < 360; deg += 10) {
			const hit = checkHit(deg, aim, from, target);
			if (hit) hitAngles.push(hit);
		}
		...
		// --- Step2: 中スキャン（5°刻み） ---
		let midAngles = [];
		for (const h of hitAngles) {
			for (let d = h.deg - 10; d <= h.deg + 10; d += 5) {
				const deg = (d + 360) % 360;
				const hit = checkHit(deg, aim, from, target);
				if (hit) midAngles.push(hit);
			}
		}

		if (midAngles.length === 0) midAngles = hitAngles;

		// --- Step3: 細スキャン（砲塔の回転速度刻み） ---
		let fineAngles = [];
		const rotSpeed = Categorys.CannonRotSpeed[category]; // ★ 砲塔の回転速度

		for (const h of midAngles) {
			for (let d = h.deg - 5; d <= h.deg + 5; d += rotSpeed) {
				const deg = (d + 360) % 360;
				const hit = checkHit(deg, aim, from, target);
				if (hit) fineAngles.push(hit);
			}
		}

		if (fineAngles.length === 0) fineAngles = midAngles;

		aim.remove();
		from.rotation = originalRot;

		// --- 命中角度の中から最適角度を選ぶ ---
		return selectBestAngle(fineAngles, originalRot);
	}

	function selectBestAngle(hitAngles, currentRot) {

		// ① 反射回数が最小の角度だけ残す
		const minReflect = Math.min(...hitAngles.map(h => h.reflectCount));
		const filtered = hitAngles.filter(h => h.reflectCount === minReflect);
		...
		for (const h of filtered) {
			const turn = Math.min(
				Math.abs(h.deg - currentRot),
				360 - Math.abs(h.deg - currentRot)
			);
			if (turn < bestTurn) {
				bestTurn = turn;
				best = h;
	...
	class BulAimLite {
		...
		constructor(from) {
			...
			// 初期位置（Sprite と同じ）
			this.x = fc.x + cos - this.w;
			this.y = fc.y + sin - this.h;
		...
		computeRay() {
			...
			const dx = this.dx / 24; // 方向ベクトル（正規化）
			const dy = this.dy / 24;

			const hit = this.raycast(x1, y1, dx, dy, this.maxLength);
			//new Point({ x: x1, y: y1 });
			//new Point({ x: hit.x, y: hit.y });
		...
		rayRect(x, y, dx, dy, rect) {
			...
			if (dy !== 0) {
				...
				const xx = x + dx * t;
				if (t > 0 && xx >= x1 && xx <= x2 && t < tMin) {
					tMin = t; hitX = xx; hitY = y2;
				}
			}

			if (tMin === Infinity) return null;

			return { x: hitX, y: hitY, dist: tMin };
		}
	...
	var TankBase = Class.create(Sprite, {
		initialize: function (x, y, category, num, scene) {
			...
			this.cannonRotSpeed = Categorys.CannonRotSpeed[this.category];
			this.distance   = Categorys.Distances[this.category];
			//	迎撃ステータス
			this.defFlg = Categorys.DefenceFlg[this.category];
			this.defRange = Categorys.DefenceRange[this.category];
			this.escRange = Categorys.EscapeRange[this.category];
			...
			this._match1 = new Set();
			this._match2 = new Set();
		...
		_DamageEffect: function () {
			...
			if (this.category === 7 && this.num > 0) {
				...
				if (useLifeSystem) this.lifeBar.opacity = 0;
			} else {
	...
	function getCloserDistance(weak, target1, target2) {
		...
		initialize: function(x, y, category, num, scene) {
			...
			this.onenterframe = function() {
				try{
					...
					console.error("Tank #" + this.num + " ERROR:", e);
				}
	...
	var Entity_Type1 = Class.create(TankBase, {
		initialize: function (x, y, category, num, scene) {
			...
			const markObstacle = (rot, myPath, grid) => {
				...
				const y = myPath[0] + off[0];
				const x = myPath[1] + off[1];

				if (y < 0 || y >= Stage_H) return;
				if (x < 0 || x >= Stage_W) return;
			...
			this.onenterframe = () => {
				try{
					...
					// 照準ヒット
					if (this.time > 60){
						const hit = this.aim.intersectsAABB(this.cursor);
		...
		_Defense: function () {
			if (BulletBase.collection.length > 0) {
				this._match1.clear();
				for (let i = 0; i < PlayerBulAimLite.list.length; i++) {
					const a = PlayerBulAimLite.list[i];
					if (a.intersectsAABB(this.around)) {
						this._match1.add(a.target); 
                		if (this._match1.size >= 6) break; // 迎撃に3本も要らない
					}
				}

				this._match2.clear();
				for (let i = 0; i < BulAimLite.list.length; i++) {
					const a = BulAimLite.list[i];
					if (a.intersectsAABB(this.around)) {
						this._match2.add(a.target); 
                		if (this._match2.size >= 6) break; // 迎撃に3本も要らない
					}
				}
				for (let i = 0, l = BulletBase.collection.length; i < l; i++) {
					...
					if ((c.num === 0 && !this.defFlg[0]) ||
						...
						(c.num !== 0 && c.num !== this.num && !this.defFlg[2])) continue;
					...
					const dist = getCloserDistance(this.weak, this.attackTarget, c);

					if (dist == null) continue;

					switch (c.num) {
						case 0:
							if (dist < this.defRange[0]) {
								if (this._match1.has(c)) {
									this.attackTarget = c;
									this.escapeFlg = true;
								}
							}
						...
						case this.num:
							if (this.ref == 0) break;
							if (dist < this.defRange[1] && dist > 100) {
								if (this._match2.has(c)) {
									this.attackTarget = c;
									this.escapeFlg = true;
								}
							}
					...
					switch (c.num) {
						case 0:
							if (dist < this.defRange[0]) {
								...
								if (this.escRange[0] && this.escRange[1] != 0) {
									if (dist < this.escRange[1]) {
										this.escapeTarget = c;
										this.escapeFlg = true;
									}
								}
						...
						case this.num:
							...
							if (dist < this.defRange[1] && dist > 100) {
								if (this._match2.has(c)) {
									if (this.escRange[0] && this.escRange[2] != 0) {
										if (dist < this.escRange[2]) {
											this.escapeTarget = c;
											this.escapeFlg = true;
										}
									}
						...
						default:
							if (dist < this.defRange[2]) {
								...
								if (this.escRange[0] && this.escRange[3] != 0) {
									if (dist < this.escRange[3]) {
										this.escapeTarget = c;
										this.escapeFlg = true;
									}
								}
					...
					switch (c.num) {
						case 0: {
							if (dist < this.defRange[0]) {
								...
								if (this.escRange[0] && this.escRange[1] !== 0 && dist < this.escRange[1]) {
									this.escapeTarget = c;
									this.escapeFlg = true;
								}
							}
						...
						case this.num: {
							if (this.ref === 0) break;
							if (dist < this.defRange[1] && dist > 100) {
								if (this._match2.has(c) && this.escRange[0] && this.escRange[2] !== 0 && dist < this.escRange[2]) {
									this.escapeTarget = c;
									this.escapeFlg = true;
								}
								if (Search(this.cannon, c, 45, this.defRange[1]) && c.time > 30) {
									this.attackTarget = c;
								}
							}
							break;
						...
						default: {
							if (dist < this.defRange[2]) {
								...
								if (this.escRange[0] && this.escRange[3] !== 0 && dist < this.escRange[3]) {
									this.escapeTarget = c;
									this.escapeFlg = true;
								}
							}
		...
		initialize: function (x, y, category, num, scene) {
			...
			this.onenterframe = () => {
				try{
					...
					const hasBest = isFinite(this.bestDeg);

					// --- 索敵ヒット時の処理 ---
					if (hit) {
						// ★ 実際に当たる角度を最優先
						this.bestDeg = this.aim.agl;
						...
						if (!this.fireFlg) {
							...
							if (this.aimHitCnt > 6 + BulletCountByTank(this.num) * 5) {
								...
								const diffFire = Math.abs(
									normalizeAngle(this.cannon.rotation - this.aim.agl)
								);

								// rotSpeed 以下なら「向いている」と判定
								if (diffFire <= Categorys.CannonRotSpeed[this.category] * 2) {
									this.fireFlg = true;
									this.cannon.rotation = this.aim.agl;
								}

								if (this.aim.agl == this.cannon2.rotation && diffFire <= Categorys.CannonRotSpeed[this.category] * 12){
									this.fireFlg = true;
									this.cannon.rotation = this.aim.agl;
								}
							}
					...
					if (this.shotStopFlg) {
						...
						if (!this.fireFlg && this.aimingTime < this.aimCmpTime) {
							...
							// --- cannon の回転処理（最適角度中心） ---
							let cannonPrev = this.cannon.rotation;
							...
							// --- cannon の回転方向を検出 ---
							const cannonDir = normalizeAngle(this.cannon.rotation - cannonPrev) > 0 ? 1 : -1;
					...
					if (!deadFlgs[this.num] && gameStatus == 0) {
						if (this.life > 0) {
							if (WorldFlg) {
								...
								// 照準ヒット
								if (this.time > 60){
									if (this.aim.intersectsAABB(this.cursor)) {
										this.fireFlg = true;
									}
									for (let i = 0; i < Bom.collection.length; i++){
										var c = Bom.collection[i];
										if (this.aim.intersectsAABB(c)) {
											this.fireFlg = false;
											break;
										}
									}
					...
					switch (c.num) {
						case 0:
							if (dist < this.defRange[0]) {
								...
								if (this.escRange[0] && this.escRange[1] != 0 && dist < this.escRange[1]) {
									if (Search(c, this, 75, this.escRange[1])) {
						...
						case this.num:
							...
							if (dist < this.defRange[1] && dist > 100) {
								if (this._match2.has(c)) {
									if (this.escRange[0] && this.escRange[2] != 0 && dist < this.escRange[2]) {
										if (Search(c, this, 60, this.escRange[2])) {
						...
						default:
							if (dist < this.defRange[2]) {
								...
								if (this.escRange[0] && this.escRange[3] != 0 && dist < this.escRange[3]) {
									if (Search(c, this, 60, this.escRange[3])) {
					...
					switch (c.num) {
						case 0:
							if (dist < this.defRange[0]) {
								...
								if (this.escRange[0] && this.escRange[1] != 0) {
									if (dist < this.escRange[1]) {
										if (Search(c, this, 80, this.escRange[1])) {
											this.escapeTarget = c;
											this.escapeFlg = true;
										}
									}
				...
				for (var i = 0, l = BulletBase.collection.length; i < l; i++) {
					...
					switch (c.num) {
						case this.target.num:
							if (dist != null && dist < this.defRange[0]) {
								if (this._match1.has(c)){
									this.attackTarget = c; //  迎撃のためにターゲット変更
								}else{
									this.attackTarget = this.target;
								}
								if (this.escRange[0] == true && this.escRange[1] != 0) {
									if (dist < this.escRange[1]) {
										if (Search(c, this, 90, this.escRange[1])) {
											this.escapeTarget = c;
											this.escapeFlg = true;
										}
									}
						...
						case this.num:
							if (this.ref == 0) break;
							if (dist != null && dist < this.defRange[1] && dist > 100) {
								if (this._match2.has(c)){
									if (this.escRange[0] == true && this.escRange[2] != 0) {
										if (dist < this.escRange[2]) {
											if (Search(c, this, 45, this.escRange[2])) {
												this.escapeTarget = c;
												this.escapeFlg = true;
											}
										}
							...
							break;
						default:
							if (dist != null && dist < this.defRange[2]) {
								if (this._match2.has(c)){
									this.attackTarget = c; //  迎撃のためにターゲット変更
								}
								if (this.escRange[0] == true && this.escRange[3] != 0) {
									if (dist < this.escRange[3]) {
										if (Search(c, this, 45, this.escRange[3])) {
											this.escapeTarget = c;
											this.escapeFlg = true;
										}
									}
	...
	var ViewRemaining = Class.create(Label, {
		initialize: function() {
			...
			this.width = useLifeSystem ? PixelSize * 12 : PixelSize * 8;
			this.height = 48;
			this.moveTo(useLifeSystem ? PixelSize * 4 : PixelSize * 6, PixelSize * 14);
			this.text = useLifeSystem
				? '敵残数：' + (tankEntity.length - 1 - destruction) + '　耐久値：' + tankEntity[0].life + '　残機：' + zanki
				: '敵残数：' + (tankEntity.length - 1 - destruction) + '　　　残機：' + zanki;
			this.font = 'bold 40px "Arial"';
			this.color = '#fffd';
			this.textAlign = 'center';
	...
	var SelWindow = Class.create(ViewArea, {
		initialize: function(position, name) {
			...
			// --- 体力制トグルボタン ---
			var lifeToggleLabel = new ViewText(this.head, 'Mode',
				{ width: 400, height: 40 },
				{ x: 32, y: 180 },
				'体力制モード：', '40px sans-serif', 'black', 'left', true);

			var lifeToggleBtn = new ViewButton(this.head, 'Mode',
				{ width: 312, height: 48 },
				{ x: 312, y: 180 },
				useLifeSystem ? 'ON（体力制）' : 'OFF（撃破制）',
				'36px sans-serif', useLifeSystem ? 'blue' : 'black', 'center',
				'rgba(0, 0, 0, 0.3)', 'rgba(0, 0, 0, 0.1)');

			var lifeDsc = new ViewText(this.head, 'Mode',
				{ width: 896, height: 60 },
				{ x: 32, y: 238 },
				useLifeSystem
					? '体力制：各戦車に体力があり、ダメージで削っていくモード。'
					: '撃破制：弾が当たれば即撃破（一部例外有）。',
				'28px sans-serif', 'black', 'left', true);

			lifeToggleBtn.addEventListener(Event.TOUCH_START, function() {
				useLifeSystem = !useLifeSystem;
				lifeToggleBtn.text.text = useLifeSystem ? 'ON（体力制）' : 'OFF（撃破制）';
				lifeToggleBtn.text.color = useLifeSystem ? 'blue' : 'black';
				lifeDsc.text = useLifeSystem
					? '体力制：各戦車に体力があり、ダメージで削っていくモード。'
					: '撃破制：弾が当たれば即撃破（一部例外有）。';
			});
	...
	var TitleScene = Class.create(Scene, {
		...
		onContinue: function () {
			...
			useLifeSystem = Repository.data.UseLife ?? useLifeSystem;
	...
	var TestScene = Class.create(Scene, {
        ...
        handleResultButtons: function () {
            ...
            if (this.time >= 120 + 15 * (this.dcnt + 5)) {
                ...
                toTitle.addEventListener(Event.TOUCH_START, function () {
                    game.stop();
                    location.href = "./game_unified.html";
                });
